...
def general_fit(equation,df,verbose=True, simplification=True, intrinsic_scatter_guess=None,
    ...
    # total number of variables with error
    total_var_with_error = 0
    for column in df.columns:
        if column.endswith('_std') and column != 'y_std' and df[column].sum() != 0:
            total_var_with_error += 1
    ...
    total_num_data = data_size*(total_var_with_error+1)
    ...
    if verbose:
        print('total_var_with_error:',total_var_with_error)
    ...
    #     if x_stds[j].sum() != 0:
    #         N_para += data_size
    N_para=np.max([number_constants,unique_number_variables])+total_var_with_error*data_size+1
    if verbose:
    ...
    result['-2LLL'] = loss.item() + loss_term4
